...
export default function JournalView() {
  ...
  // Fetch posts from /posts.json — generated by GitHub Actions every 10 min from Wix RSS.
  // Same-origin fetch: zero CORS issues, zero proxy caching, always up to date.
  useEffect(() => {
    async function fetchPosts() {
      try {
        const res = await fetch(`/GBPublishing_newsite/posts.json?t=${Date.now()}`, { cache: 'no-store' });
        if (res.ok) {
        ...
        console.warn("posts.json fetch failed, showing defaults:", err);
      }
    }
    fetchPosts();
  }, []);
